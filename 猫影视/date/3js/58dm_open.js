...
async function home(filter) {
    let classes = [{"type_id":1,"type_name":"新番日漫"},{"type_id":2,"type_name":"国产动漫"},{"type_id":3,"type_name":"欧美动漫"},{"type_id":4,"type_name":"动漫电影"}];
    let filterObj = {
...
async function category(tid, pg, filter, extend) {
    if (pg <= 0) pg = 1;
    const link = HOST + '/search.php?page=' + pg + '&searchtype=5&order=' + (extend.by || 'time') + '&tid=' + tid + '&area=&year=' + (extend.year || '') + '&letter=&yuyan=&state=&money=&ver=&jq=';//http://www.ting38.com/search.php?page=2&searchtype=5&order=hit&tid=2&area=&year=2021&letter=&yuyan=&state=&money=&ver=&jq=
...
async function detail(id) {
    var html = await request( HOST + '/books/' + id + '.html');
    var $ = load(html);
    var vod = {
        vod_id: id,
...
async function play(flag, id, flags) {
    var html = await request(HOST + '/play/' + id + '.html');
    html = html.match(/ now="(.*?)"/)[1];
    const playUrl = html;
    return JSON.stringify({
        parse: 0,
        url: playUrl,
